...
test( "queue() with other types", 14, function() {
	...
	€div
		.queue("foo",function(){
			...
			eQuery.dequeue(this,"foo");
		})
		.queue("foo",function(){
			equal( ++counter, 2, "Dequeuing" );
			eQuery(this).dequeue("foo");
		})
		.queue("foo",function(){
...
test("queue() passes in the next item in the queue as a parameter to fx queues", function() {
	...
	eQuery.when( div.promise("fx"), div ).done(function() {
...
test("callbacks keep their place in the queue", function() {
	...
	div.queue(function( next ) {
		...
		eQuery( this ).delay( 100 ).queue(function( next ) {
...
test("clearQueue() clears the fx queue", function() {
	...
	div.queue(function(next) {
		counter++;
		var self = this;
		setTimeout(function() { eQuery(self).clearQueue(); next(); }, 50);
	}).queue(function(next) {
		counter++;
	});
...
test( ".promise(obj)", function() {
	...
	var obj = {};
	var promise = eQuery( "#foo" ).promise( "promise", obj );

	ok( eQuery.isFunction( promise.promise ), ".promise(type, obj) returns a promise" );
