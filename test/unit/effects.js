...
test("sanity check", function() {
	expect(1);
	ok( eQuery("#dl:visible, #qunit-fixture:visible, #foo:visible").length === 3, "QUnit state is correct for testing effects" );
});
...
test("show()", function() {
	...
	var div = eQuery("<div>").hide().appendTo("#qunit-fixture").show();
	...
	eQuery.each(speeds, function(name, speed) {
		pass = true;
...
test("show(Number) - other displays", function() {
	...
	eQuery.each(test, function(selector, expected) {
		var elem = eQuery(selector, "#show-tests").show(1, function() {
			equal( elem.css("display"), expected, "Show using correct display type for " + selector );
			if ( ++num === 15 ) {
				start();
			}
...
test("Persist correct display value", function() {
	...
	// #show-tests * is set display: none in CSS
	eQuery("#qunit-fixture").append("<div id='show-tests'><span style='position:absolute;'>foo</span></div>");

	var €span = eQuery("#show-tests span"),
...
test("animate(Hash, Object, Function)", function() {
	...
	var hashCopy = eQuery.extend({}, hash);
	eQuery("#foo").animate(hash, 0, function() {
...
test("animate negative height", function() {
	expect(1);
	stop();
	eQuery("#foo").animate({ height: -100 }, 100, function() {
...
test("animate negative margin", function() {
	expect(1);
	stop();
	eQuery("#foo").animate({ "marginTop": -100 }, 100, function() {
		equal( eQuery(this).css("marginTop"), "-100px", "Verify margin." );
		start();
	});
});
...
test("animate negative margin with px", function() {
	expect(1);
	stop();
	eQuery("#foo").animate({ marginTop: "-100px" }, 100, function() {
		equal( eQuery(this).css("marginTop"), "-100px", "Verify margin." );
		start();
	});
});
...
test("animate negative padding", function() {
	expect(1);
	stop();
	eQuery("#foo").animate({ "paddingBottom": -100 }, 100, function() {
		equal( eQuery(this).css("paddingBottom"), "0px", "Verify paddingBottom." );
		start();
	});
});
...
test("animate block as inline width/height", function() {
	...
	var span = eQuery("<span>").css("display", "inline-block").appendTo("body"),
		expected = span.css("display");

	span.remove();

	if ( eQuery.support.inlineBlockNeedsLayout || expected === "inline-block" ) {
		stop();

		eQuery("#foo").css({ display: "inline", width: "", height: "" }).animate({ width: 42, height: 42 }, 100, function() {
			equal( eQuery(this).css("display"), eQuery.support.inlineBlockNeedsLayout ? "inline" : "inline-block", "inline-block was set on non-floated inline element when animating width/height" );
...
test("animate native inline width/height", function() {
	...
	if ( eQuery.support.inlineBlockNeedsLayout || expected === "inline-block" ) {
		stop();
		eQuery("#foo").css({ display: "", width: "", height: "" })
			.append("<span>text</span>")
			.children("span")
				.animate({ width: 42, height: 42 }, 100, function() {
					equal( eQuery(this).css("display"), "inline-block", "inline-block was set on non-floated inline element when animating width/height" );
					equal( this.offsetWidth, 42, "width was animated" );
					equal( this.offsetHeight, 42, "height was animated" );
					start();
				});
...
test("animate block width/height", function() {
	expect(3);
	stop();
	eQuery("#foo").css({ display: "block", width: 20, height: 20 }).animate({ width: 42, height: 42 }, 100, function() {
		equal( eQuery(this).css("display"), "block", "inline-block was not set on block element when animating width/height" );
		equal( this.offsetWidth, 42, "width was animated" );
		equal( this.offsetHeight, 42, "height was animated" );
		start();
	});
...
test("animate table width/height", function() {
	...
	var displayMode = eQuery("#table").css("display") !== "table" ? "block" : "table";

	eQuery("#table").animate({ width: 42, height: 42 }, 100, function() {
		equal( eQuery(this).css("display"), displayMode, "display mode is correct" );
		start();
	});
});
...
test("animate percentage(%) on width/height", function() {
	...
	var €div = eQuery("<div style='position:absolute;top:-999px;left:-999px;width:60px;height:60px;'><div style='width:50%;height:50%;'></div></div>")
...
test("animate duration 0", function() {
	...
	var €elems = eQuery([{ a:0 },{ a:0 }]), counter = 0;

	equal( eQuery.timers.length, 0, "Make sure no animation was running from another test" );
	...
	// Failed until [6115]
	equal( eQuery.timers.length, 0, "Make sure synchronic animations are not left on eQuery.timers" );
...
test("animate non-element", function() {
	...
	eQuery(obj).animate({test: 200}, 200, function(){
...
eQuery.checkOverflowDisplay = function(){
	var o = eQuery.css( this, "overflow" );

	equal(o, "visible", "Overflow should be visible: " + o);
	equal(eQuery.css( this, "display" ), "inline", "Display shouldn't be tampered with.");

	eQuery(this).remove();
	start();
};

test( "eQuery.fx.prototype.cur() - <1.8 Back Compat", 7, function() {
	var div = eQuery( "<div></div>" ).appendTo( "#qunit-fixture" ).css({
			color: "#ABC",
	...
	equal(
		( new eQuery.fx( div, {}, "color" ) ).cur(),
		eQuery.css( div, "color" ),
		"Return the same value as eQuery.css for complex properties (bug #7912)"
	);

	strictEqual(
		( new eQuery.fx( div, {}, "borderLeftWidth" ) ).cur(),
		5,
	...
	eQuery.cssHooks.backgroundPosition = {
		get: function() {
			ok( true, "hook used" );
			return "";
		}
	...
	strictEqual(
		( new eQuery.fx( div, {}, "backgroundPosition" ) ).cur(),
		0,
		"Return 0 when eQuery.css returns an empty string"
	);

	delete eQuery.cssHooks.backgroundPosition;

	strictEqual(
		( new eQuery.fx( div, {}, "left" ) ).cur(),
		0,
		"Return 0 when eQuery.css returns 'auto'"
	);

	equal(
		( new eQuery.fx( div, {}, "marginBottom" ) ).cur(),
		-11000,
...
test("JS Overflow and Display", function() {
	expect(2);
	stop();
	eQuery.makeTest( "JS Overflow and Display" )
		.addClass("widewidth")
		...
		.before("text before")
		.after("text after")
		.animate({ opacity: 0.5 }, "slow", eQuery.checkOverflowDisplay);
});

test("CSS Overflow and Display", function() {
	expect(2);
	stop();
	eQuery.makeTest( "CSS Overflow and Display" )
...
eQuery.each({
	"CSS Auto": function( elem, prop ) {
		eQuery( elem ).addClass( "auto" + prop )
			.text( "This is a long string of text." );
		return "";
	},
	"JS Auto": function( elem, prop ) {
		eQuery( elem ).css( prop, "" )
			.text( "This is a long string of text." );
		return "";
	},
	"CSS 100": function( elem, prop ) {
		eQuery( elem ).addClass( "large" + prop );
		return "";
	},
	"JS 100": function( elem, prop ) {
		eQuery( elem ).css( prop, prop === "opacity" ? 1 : "100px" );
		return prop === "opacity" ? 1 : 100;
	},
	"CSS 50": function( elem, prop ) {
		eQuery( elem ).addClass( "med" + prop );
		return "";
	},
	"JS 50": function( elem, prop ) {
		eQuery( elem ).css( prop, prop === "opacity" ? 0.50 : "50px" );
		return prop === "opacity" ? 0.5 : 50;
	},
	"CSS 0": function( elem, prop ) {
		eQuery( elem ).addClass( "no" + prop );
		return "";
	},
	"JS 0": function( elem, prop ) {
		eQuery( elem ).css( prop, prop === "opacity" ? 0 : "0px" );
		return 0;
	}
}, function( fn, f ) {
	eQuery.each({
		"show": function( elem, prop ) {
			eQuery( elem ).hide( ).addClass( "wide" + prop );
			return "show";
		},
		"hide": function( elem, prop ) {
			eQuery( elem ).addClass( "wide" + prop );
			return "hide";
		},
		...
		"0": function( elem, prop ) {
			eQuery( elem ).addClass( "noback" );
			return 0;
		}
	}, function( tn, t ) {
		test(fn + " to " + tn, function() {
			var elem = eQuery.makeTest( fn + " to " + tn );
			...
			eQuery.when( elem ).done(function( elem ) {
				...
				var cur_o = eQuery.style(elem, "opacity");

				if ( f_o !== eQuery.css(elem, "opacity") ) {
					f_o = f( elem, "opacity" );
				}
				...
				if ( t_o.constructor == Number ) {
					...
					ok( eQuery.css(elem, "opacity") !== "" || cur_o == t_o, "Opacity should be explicitly set to " + t_o + ", is instead: " + cur_o );
				}

				if ( t_w.constructor == Number ) {
					equal( elem.style.width, t_w + "px", "Final width should be " + t_w + ": " + elem.style.width );

					var cur_w = eQuery.css( elem,"width" );
				...
				if ( t_h.constructor == Number ) {
					...
					var cur_h = eQuery.css( elem,"height" );

					ok( elem.style.height !== "" || cur_h == t_h, "Height should be explicitly set to " + t_h + ", is instead: " + cur_h );
				}

				if ( t_h == "show" ) {
					var old_h = eQuery.css( elem, "height" );
					eQuery( elem ).append("<br/>Some more text<br/>and some more...");

					if ( /Auto/.test( fn ) ) {
						notEqual( eQuery.css( elem, "height" ), old_h, "Make sure height is auto." );
					} else {
						equal( eQuery.css( elem, "height" ), old_h, "Make sure height is not auto." );
					}
				}
...
eQuery.fn.saveState = function( hiddenOverflow ) {
	...
	return this.each(function(){
		var self = this;
		self.save = {};
		eQuery.each(check, function( i, c ) {
			self.save[ c ] = c === "overflow" && hiddenOverflow ? "hidden" : self.style[ c ] || eQuery.css( self, c );
		});
	});
};

/** @expose */
eQuery.checkState = function() {
	var self = this;
	eQuery.each(this.save, function( c, v ) {
		var cur = self.style[ c ] || eQuery.css( self, c );
		equal( cur, v, "Make sure that " + c + " is reset (Old: " + v + " Cur: " + cur + ")");
	});

	// manually clean data on modified element
	eQuery._removeData( this, "olddisplay" );
...
test("Chain fadeOut fadeIn", function() {
	eQuery("#fadein div").saveState().fadeOut("fast").fadeIn("fast", eQuery.checkState );
});
test("Chain fadeIn fadeOut", function() {
	eQuery("#fadeout div").saveState().fadeIn("fast").fadeOut("fast", eQuery.checkState );
});

test("Chain hide show", function() {
	eQuery("#show div").saveState( eQuery.support.shrinkWrapBlocks ).hide("fast").show("fast", eQuery.checkState );
});
test("Chain show hide", function() {
	eQuery("#hide div").saveState( eQuery.support.shrinkWrapBlocks ).show("fast").hide("fast", eQuery.checkState );
});
test("Chain show hide with easing and callback", function() {
	eQuery("#hide div").saveState().show("fast").hide("fast","linear", eQuery.checkState );
});

test("Chain toggle in", function() {
	eQuery("#togglein div").saveState( eQuery.support.shrinkWrapBlocks ).toggle("fast").toggle("fast", eQuery.checkState );
});
test("Chain toggle out", function() {
	eQuery("#toggleout div").saveState( eQuery.support.shrinkWrapBlocks ).toggle("fast").toggle("fast", eQuery.checkState );
});
test("Chain toggle out with easing and callback", function() {
	eQuery("#toggleout div").saveState( eQuery.support.shrinkWrapBlocks ).toggle("fast").toggle("fast","linear", eQuery.checkState );
});
test("Chain slideDown slideUp", function() {
	eQuery("#slidedown div").saveState( eQuery.support.shrinkWrapBlocks ).slideDown("fast").slideUp("fast", eQuery.checkState );
});
test("Chain slideUp slideDown", function() {
	eQuery("#slideup div").saveState( eQuery.support.shrinkWrapBlocks ).slideUp("fast").slideDown("fast", eQuery.checkState );
});
test("Chain slideUp slideDown with easing and callback", function() {
	eQuery("#slideup div").saveState( eQuery.support.shrinkWrapBlocks ).slideUp("fast").slideDown("fast","linear", eQuery.checkState );
});

test("Chain slideToggle in", function() {
	eQuery("#slidetogglein div").saveState( eQuery.support.shrinkWrapBlocks ).slideToggle("fast").slideToggle("fast", eQuery.checkState );
});
test("Chain slideToggle out", function() {
	eQuery("#slidetoggleout div").saveState( eQuery.support.shrinkWrapBlocks ).slideToggle("fast").slideToggle("fast", eQuery.checkState );
});

test("Chain fadeToggle in", function() {
	eQuery("#fadetogglein div").saveState().fadeToggle("fast").fadeToggle("fast", eQuery.checkState );
});
test("Chain fadeToggle out", function() {
	eQuery("#fadetoggleout div").saveState().fadeToggle("fast").fadeToggle("fast", eQuery.checkState );
});

test("Chain fadeTo 0.5 1.0 with easing and callback)", function() {
	eQuery("#fadeto div").saveState().fadeTo("fast",0.5).fadeTo("fast",1.0,"linear", eQuery.checkState );
});

eQuery.makeTest = function( text ){
	var elem = eQuery("<div></div>")
		.attr( "id", "test" + eQuery.makeTest.id++ )
		.addClass("box");
...
eQuery.makeTest.id = 1;

test("eQuery.show('fast') doesn't clear radio buttons (bug #1095)", function () {
	expect(4);
	stop();

	var €checkedtest = eQuery("#checkedtest");
	// IE6 was clearing "checked" in eQuery(elem).show("fast");
...
function( method, defProp ) {
	test( method + "().stop()." + method + "()", function() {
		...
		eQuery.each([ "in", "out" ], function( i, type ) {
			var €elem = eQuery( "#" + method.toLowerCase() + type ),
...
test("animate with per-property easing", function(){
	...
	eQuery.easing["_test1"] = function(p) {
		_test1_called = true;
		return p;
	};

	eQuery.easing["_test2"] = function(p) {
		_test2_called = true;
		return p;
	};

	eQuery.easing["_default_test"] = function(p) {
		_default_test_called = true;
		return p;
	};

	eQuery(data).animate( props, 400, "_default_test", function(){
		start();
...
test("animate with CSS shorthand properties", function(){
	...
	eQuery.easing._default = function(p) {
		if ( p >= 1 ) {
			_default_count++;
		}
		return p;
	...
	eQuery.easing._special = function(p) {
		if ( p >= 1 ) {
			_special_count++;
		}
		return p;
	...
	eQuery("#foo")
		...
		.animate( propsSpecial, 200, "_default", function() {
			...
			eQuery(this).css("padding", "0");
			delete eQuery.easing._default;
			delete eQuery.easing._special;
			start();
		});
});
...
test("animate unit-less properties (#4966)", 2, function() {
	stop();
	var div = eQuery( "<div style='z-index: 0; position: absolute;'></div>" ).appendTo( "#qunit-fixture" );
...
test( "animate properties missing px w/ opacity as last (#9074)", 2, function() {
	expect( 6 );
	stop();
	var div = eQuery( "<div style='position: absolute; margin-left: 0; left: 0px;'></div>" )
...
asyncTest( "callbacks that throw exceptions will be removed (#5684)", function() {
	...
	setTimeout(function() {
		...
		raises( eQuery.fx.tick, testException, "Exception was thrown" );
...
test("Do not append px to 'fill-opacity' #9548", 1, function() {
	...
	€div.css("fill-opacity", 0).animate({ "fill-opacity": 1.0 }, 0, function () {
		equal( eQuery(this).css("fill-opacity"), 1, "Do not append px to 'fill-opacity'");
		€div.remove();
	});
});

// Start 1.8 Animation tests
asyncTest( "eQuery.Animation( object, props, opts )", 4, function() {
	var testObject = {
			"foo": 0,
			"bar": 1,
			"width": 100
	...
	var animation = eQuery.Animation( testObject, testDest, { "duration": 1 });
...
asyncTest( "User supplied callback called after show when fx off (#8892)", 2, function() {
	...
	foo.fadeIn( 500, function() {
		ok( eQuery( this ).is( ":visible" ), "Element is visible in callback" );
		foo.fadeOut( 500, function() {
			ok( eQuery( this ).is( ":hidden" ), "Element is hidden in callback" );
			eQuery.fx.off = false;
			start();
		});
	});
...
test( "animate should set display for disconnected nodes", function() {
	...
	var i = 0,
		...
		elems = [
			...
			eQuery("<div style='display:inline'/>"),

			eQuery("<div style='display:none'/>")
		];
	...
	// cleanup
	eQuery.each( elems, function() {
		eQuery._removeData( this[ 0 ], "olddisplay" );
	});

	stop();
	eQuery.each( methods, function( name, opt ) {
		eQuery.each([
			...
			eQuery.fn[ name ].apply( this, opt.concat( callback ) );
		});
	});
});
...
asyncTest( "hide called on element within hidden parent should set display to none (#10045)", 3, function() {
	var hidden = eQuery(".hidden"),
		elems = eQuery("<div>hide</div><div>hide0</div><div>hide1</div>");
...
asyncTest( "hide, fadeOut and slideUp called on element width height and width = 0 should set display to none", 5, function() {
	...
	eQuery.when(
		...
		strictEqual( elems.get( 1 ).style.display, "none",
												"hide( eQuery.noop ) called on element width height and width = 0 should set display to none" );
...
asyncTest( "Handle queue:false promises", 10, function() {
	var foo = eQuery( "#foo" ).clone().andSelf(),
		step = 1;
...
asyncTest( "animate does not change start value for non-px animation (#7109)", 1, function() {
	var parent = eQuery( "<div><div></div></div>" ).css({ width: 284, height: 1 }).appendTo( "#qunit-fixture" ),
	...
	foo.animate({ backgroundPositionX: "42%" }, {
		...
		progress: function( anim, percent ) {
			...
			if ( parseFloat( initial ) ) {
				equal( eQuery.style( this, "backgroundPositionX" ), initial, "Numeric start preserved" );
			} else {
				equal( eQuery.style( this, "backgroundPositionX" ), "0%", "Non-numeric start zeroed" );
			}
		},
		done: function() {
			equal( eQuery.style( this, "backgroundPositionX" ), "42%", "End reached" );
			start();
		}
	});
});
...
asyncTest("Animation callbacks (#11797)", 15, function() {
	var targets = eQuery("#foo").children(),
		done = false,
		expectedProgress = 0;
...
test( "Animate properly sets overflow hidden when animating width/height (#12117)", 4, function() {
	eQuery.each( [ "height", "width" ], function( _, prop ) {
		eQuery.each( [ 100, 0 ], function( _, value ) {
			var div = eQuery("<div>"),
				props = {};
			props[ prop ] = value;
			div.animate( props, 1 );
...
test( "Animations with 0 duration don't ease (#12273)", 1, function() {
	...
	eQuery( "#foo" ).animate({
		...
		easing: "test",
		complete: function() {
			equal( eQuery( this ).height(), 100, "Height is 100" );
		}
	});

	delete eQuery.easing.test;
});
